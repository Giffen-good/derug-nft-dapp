...
import { ConfirmBurnToken } from '../lib/ConfirmBurnToken';
import {
    getImage,
    getName,
} from '../lib/utilities';
...
function BurnOverlay(props: BurnOverlayProps) {
    ...
    return (
        <div
            style={{
                backgroundColor: `rgba(255, 97, 99, ${markForBurn ? 0.9 : 1})`,
                position: 'absolute',
                zIndex: 2,
                bottom: 0,
                width: '210px',
                height: markForBurn
                    ? burnMode === BurnMode.BurnNfts
                        ? '210px'
                        : '230px'
                    : '40px',
                marginLeft: '20px',
...
function WalletContents(props: WalletContentProps) {
    const {
        nfts,
        setNfts,
        tokenMap,
    ...
    const data = React.useMemo(() => {
                            ...
                            <div className='row-item' key={nft.mint}>
                                ...
                                >
                                    <div
                                        style={{
                                            width: '210px',
                                            height: burnMode === BurnMode.BurnNfts
                                                ? '210px'
                                                : '230px',
                                            marginLeft: '20px',
                                            marginRight: '20px',
                                    ...
                                    >
                                        <img
                                            ...
                                            src={url}
                                            style={{
                                                maxWidth: burnMode === BurnMode.BurnNfts
                                                    ? '210px'
                                                    : '160px',
                                                maxHeight: burnMode === BurnMode.BurnNfts
                                                    ? '210px'
                                                    : '160px',
                                                borderRadius: burnMode === BurnMode.BurnNfts
                                                    ? '2px'
                                                    : '50%',
                                                marginTop: burnMode === BurnMode.BurnNfts
                                                    ? '0px'
                                                    : '15px',
                                                objectFit: 'contain',
                                            }}
                                ...
                                <div style={{
                                    ...
                                    <span style={{
                                        fontSize,
                                        color: 'white',
                                        /* Don't capitalize mints, makes it harder to match up */
                                        textTransform: burnMode === BurnMode.BurnUnknownTokens
                                            ? undefined
                                            : 'uppercase',
                                        overflow: 'hidden',
                                        textAlign: 'center',
                                        textOverflow: 'ellipsis',
                                        wordBreak: burnMode === BurnMode.BurnUnknownTokens
                                            ? 'break-all'
                                            : 'break-word'
                                    }}>
                                        {name}
        ...
        if (pageCount === 0) {
            return (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', marginTop: '30px', marginBottom: '30px' }}>
                    <span style={{ color: 'white', fontSize: '30px', marginLeft: '10px', marginRight: '10px' }}>
