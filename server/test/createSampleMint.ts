...
import {getUpdateAuthorityWallet} from "../lib/utils";
...
export const createSampleMint = async (): Promise<any> => {
    ...
    for (let j = 0; j < MULTIPLIER; j++) {
        for (let i = 0; i < meta.length;i++) {
            const ipfsPin = meta[i];
            const {updateAuthorityKeypair} = getUpdateAuthorityWallet();
            const {mintIx, mint}  = await createMintTx( connection, updateAuthorityKeypair.publicKey, ipfsPin );
